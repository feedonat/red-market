...
export class FirebaseSignInPage implements OnDestroy {
  ...
  redirectLoggedUserToProfilePage(data,mode) {
    ...
    this.ngZone.run(() => {
      ...
      const previousUrl = 'redmarket/items';
  ...
  doTwitterLogin(): void {
    ...
    .subscribe((result) => {
      ...
      // var token = result.credential.accessToken;
      this.redirectLoggedUserToProfilePage(result,'twitter');
    }, (error) => {
      console.log(error);
      this.manageAuthWithProvidersErrors(error.message);
    });
  ...
  signUp(){
    this.router.navigate(["/redmarket/profile/auth/sign-up"], { replaceUrl: true });
  }
}
